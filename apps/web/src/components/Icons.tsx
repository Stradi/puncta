...
function BaseIcon({ size = 'md', stroke = 'medium', fillColor, svgClassName, children, className, ...props }: BaseIconProps) {
  return (
    <i {...props} className={className}>
      <svg
        ...
        fill={fillColor || 'none'}
        viewBox="0 0 24 24"
        strokeWidth={cn(stroke == 'thinner' && 0.5, stroke == 'thin' && 1, stroke == 'medium' && 1.5, stroke == 'thick' && 2, stroke == 'thicker' && 4)}
        stroke="currentColor"
        className={cn(size == 'sm' && 'h-4 w-4', size == 'md' && 'h-6 w-6', size == 'lg' && 'h-8 w-8', size == '2xl' && 'h-16 w-16', svgClassName)}
      >
        {children}
      </svg>
    </i>
